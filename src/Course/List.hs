...
map ::
  ...
  -> List a
  -> List b
map fab = foldRight (\a lst -> fab a :. lst ) Nil
...
filter ::
  ...
  -> List a
  -> List a
filter fab = foldRight (t fab) Nil
  where t f x l | f x == True = x :. l |otherwise = l
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional = foldRight t (Full Nil)
  where t Empty _ = Empty
        t _ Empty = Empty
        t (Full x) (Full lst) = Full(x :. lst)
